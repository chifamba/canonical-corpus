...
func (d *Downloader) Fetch(ctx context.Context, rawURL string) ([]byte, string, error) {
	return d.fetchWithHeaders(ctx, rawURL, nil)
}

// FetchSource downloads a URL described by a SourceRef, applying any API-key
// authentication defined on the source (AuthHeader / AuthEnvVar / AuthPrefix).
func (d *Downloader) FetchSource(ctx context.Context, src metadata.SourceRef) ([]byte, string, error) {
	var extraHeaders map[string]string
	if src.AuthHeader != "" && src.AuthEnvVar != "" {
		keyValue := os.Getenv(src.AuthEnvVar)
		if keyValue != "" {
			extraHeaders = map[string]string{
				src.AuthHeader: src.AuthPrefix + keyValue,
			}
		} else {
			d.logger.Warn("API key env var not set, skipping auth header",
				zap.String("env_var", src.AuthEnvVar),
				zap.String("url", src.URL))
		}
	}
	return d.fetchWithHeaders(ctx, src.URL, extraHeaders)
}

// fetchWithHeaders is the shared implementation used by Fetch and FetchSource.
func (d *Downloader) fetchWithHeaders(ctx context.Context, rawURL string, extraHeaders map[string]string) ([]byte, string, error) {
	...
	for attempt := 0; attempt <= d.config.MaxRetries; attempt++ {
		...
		body, contentType, fetchErr = d.doFetch(ctx, rawURL, extraHeaders)
...
func (d *Downloader) doFetch(ctx context.Context, rawURL string, extraHeaders map[string]string) ([]byte, string, error) {
