...
// Write persists a document as <baseDir>/<category>/<NNN-title>/<filename>.md.
// The filename is derived from the document's language and translation:
//   - Single-language result: "<language>.md" (e.g. "he.md", "el.md")
//   - English with translation code: "<language>-<translation>.md" (e.g. "en-esv.md", "en-niv.md")
func (w *Writer) Write(doc *metadata.Document) error {
	...
	filename := BuildFilename(doc.Meta.Language, doc.Meta.Translation)
...
func BuildFilename(language, translation string) string {
	...
	if translation != "" {
		return language + "-" + strings.ToLower(translation) + ".md"
	}
	return language + ".md"
}
...
func FormatFrontmatter(meta metadata.BookMeta) string {
	...
	for _, s := range meta.Sources {
		sources = append(sources, frontmatterSource{
			URL:         s.URL,
			Format:      s.Format,
...
func BuildMarkdown(doc *metadata.Document) string {
	...
	if doc.Meta.Translation != "" {
		sb.WriteString(fmt.Sprintf("- **Translation**: %s\n", doc.Meta.Translation))
	}
